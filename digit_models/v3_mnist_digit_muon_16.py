...
====================
MNIST digit ensemble — OCRConvNetMuon, Muon optimizer, 16x16
...
Architecture — OCRConvNetMuon (new for v3):
  Filter progression: 64→128→256→512, plain residual blocks (Conv-BN-
  GELU x2 + shortcut) — no SE attention, no stochastic depth. Kept
  deliberately simpler than OCRConvNetWide (AdamW's architecture):
  Muon's own value proposition here is convergence speed per step, not
  requiring an elaborate architecture to show a benefit.
...
2026-07-27 and 2026-08-08 entries for both sides of that history):
  ...
  other first-order-backward optimizer, AdamW) — unlike SOAP, Muon's
  Newton-Schulz iteration has no dependency on float32-only computation,
  so there is no reason to disable AMP here.
...
Output: ./v3_mnist_digit_muon_16/  (created next to this script)
"""
...
OUTPUT_ROOT   = Path(__file__).resolve().parent / f"v3_mnist_digit_muon_{IMG_SIZE}"


def build_config(img_size: int, batch_size: int) -> dict:
    prefix = f"v3_mnist_digit_muon_{img_size}"
    return {
        "img_size":    img_size,
...
def train_one_epoch(model, loader, criterion, optimizer, scaler, device, scheduler,
    ...
    for batch_idx, (images, labels) in enumerate(loader):
        ...
        if _stepped:
            # Skip advancing the LR schedule on a batch where GradScaler
            # skipped the underlying optimizer.step() (inf/nan gradient,
            # most common during the first several batches while the loss
            # scale is still calibrating).
            scheduler.step()
...
def run_training(img_size: int, batch_override: int = None, gpu_id: int = None):
    ...
    print("=" * 60)
    print(f"  MNIST Digit Ensemble — Muon  [{img_size}x{img_size}]")
    ...
    plot_history(history, cfg["plot_path"], img_size, title="MNIST Digit Ensemble Muon")
    if is_main_process():
...
if __name__ == "__main__":
    ...
    OUTPUT_ROOT.mkdir(parents=True, exist_ok=True)
    sys.stdout = _Tee(OUTPUT_ROOT / f"v3_mnist_digit_muon_{IMG_SIZE}_cli_{datetime.now().strftime('%Y%m%d_%H%M%S')}.txt")
