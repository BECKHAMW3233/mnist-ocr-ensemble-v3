...
====================
MNIST digit ensemble — OCRConvNetWide, Schedule-Free AdamW, 128x128
Pure PyTorch — Wider architecture with Squeeze-Excitation attention blocks.
Adapted from v2's mnist_adamw_128.py (32/64/128 tiers) — the
...
Output: ./v3_mnist_digit_adamw_128/  (created next to this script)
"""
...
OUTPUT_ROOT      = Path(__file__).resolve().parent / f"v3_mnist_digit_adamw_{IMG_SIZE}"

LABEL_MAP = list("0123456789")


def build_config(img_size: int, batch_size: int) -> dict:
    prefix = f"v3_mnist_digit_adamw_{img_size}"
    return {
        "img_size":    img_size,
...
if __name__ == "__main__":
    ...
    OUTPUT_ROOT.mkdir(parents=True, exist_ok=True)
    sys.stdout = _Tee(OUTPUT_ROOT / f"v3_mnist_digit_adamw_{IMG_SIZE}_cli_{datetime.now().strftime('%Y%m%d_%H%M%S')}.txt")
