...
====================
Uppercase letter-identity ensemble — OCRConvNetWide, Schedule-Free AdamW,
128x128. Pure PyTorch — wider architecture with Squeeze-Excitation attention
blocks. Same architecture and hyperparameters as v3_mnist_digit_adamw_128.py
...
v3_mnist_digit_adamw_128.py — see that file's own docstring /
...
already returns — the same pattern v3_mnist_digit_soap_128.py /
v3_mnist_digit_muon_128.py already use for their own no-supplementary-data
...
Output: ./v3_mnist_letter_uc_adamw_128/  (created next to this script)
"""
...
OUTPUT_ROOT      = Path(__file__).resolve().parent / f"v3_mnist_letter_uc_adamw_{IMG_SIZE}"

LABEL_MAP = list("ABCDEFGHIJKLMNOPQRSTUVWXYZ")


def build_config(img_size: int, batch_size: int) -> dict:
    prefix = f"v3_mnist_letter_uc_adamw_{img_size}"
    return {
        "img_size":    img_size,
...
def run_training(img_size: int, batch_override: int = None, gpu_id: int = None):
    ...
    print("=" * 60)
    print(f"  Letter Identity Ensemble — Uppercase — ScheduleFree-AdamW  [{img_size}x{img_size}]")
    ...
    plot_history(history, cfg["plot_path"], img_size, title="Letter Identity Ensemble (Uppercase) ScheduleFree-AdamW")
    if is_main_process():
...
def main(batch_override=None, gpu_id=None):
    ...
    print("\n" + "#" * 60)
    print("  TRAINING COMPLETE — SCHEDULEFREE-ADAMW (UPPERCASE)")
...
if __name__ == "__main__":
    ...
    OUTPUT_ROOT.mkdir(parents=True, exist_ok=True)
    sys.stdout = _Tee(OUTPUT_ROOT / f"v3_mnist_letter_uc_adamw_{IMG_SIZE}_cli_{datetime.now().strftime('%Y%m%d_%H%M%S')}.txt")
