...
====================
Lowercase letter-identity ensemble — OCRConvNetMuon, Muon optimizer, 128x128.
Pure PyTorch. Same architecture and hyperparameters as
v3_mnist_digit_muon_128.py (the digit ensemble), retargeted at a 26-class
...
original "implemented directly here" design, matching v3_mnist_digit_muon_128.py's
own convention; see v3_CHANGELOG.md's 2026-07-27 and 2026-08-08 entries
for both sides of that history). Identical hyperparameters to v3_mnist_digit_muon_128.py —
see that file's own
...
Output: ./v3_mnist_letter_lc_muon_128/  (created next to this script)
"""
...
OUTPUT_ROOT   = Path(__file__).resolve().parent / f"v3_mnist_letter_lc_muon_{IMG_SIZE}"


def build_config(img_size: int, batch_size: int) -> dict:
    prefix = f"v3_mnist_letter_lc_muon_{img_size}"
    return {
        "img_size":    img_size,
...
#     was previously implemented directly in this file, matching v3_mnist_digit_muon_128.py
...
def run_training(img_size: int, batch_override: int = None, gpu_id: int = None):
    ...
    print("=" * 60)
    print(f"  Letter Identity Ensemble — Lowercase — Muon  [{img_size}x{img_size}]")
    ...
    plot_history(history, cfg["plot_path"], img_size, title="Letter Identity Ensemble (Lowercase) Muon")
    if is_main_process():
...
def main(batch_override=None, gpu_id=None):
    ...
    print("\n" + "#" * 60)
    print("  TRAINING COMPLETE — MUON (LOWERCASE)")
...
if __name__ == "__main__":
    ...
    OUTPUT_ROOT.mkdir(parents=True, exist_ok=True)
    sys.stdout = _Tee(OUTPUT_ROOT / f"v3_mnist_letter_lc_muon_{IMG_SIZE}_cli_{datetime.now().strftime('%Y%m%d_%H%M%S')}.txt")
