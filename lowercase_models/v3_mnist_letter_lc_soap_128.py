...
====================
Lowercase letter-identity ensemble — OCRConvNetTriplePyramid, SOAP
...
lowercase a-z letter-identity problem — see v3_CHANGELOG.md for the full
...
Output: ./v3_mnist_letter_lc_soap_128/  (created next to this script)
"""
...
OUTPUT_ROOT   = Path(__file__).resolve().parent / f"v3_mnist_letter_lc_soap_{IMG_SIZE}"

DEVICE = torch.device("cuda" if torch.cuda.is_available() else "cpu")


def build_config(img_size: int, batch_size: int) -> dict:
    prefix = f"v3_mnist_letter_lc_soap_{img_size}"
    return {
        "img_size":    img_size,
...
def run_training(img_size: int, batch_override: int = None, gpu_id: int = None):
    ...
    print("=" * 60)
    print(f"  Letter Identity Ensemble — Lowercase — SOAP  [{img_size}x{img_size}]")
...
def main(batch_override=None, gpu_id=None):
    ...
    print("\n" + "#" * 60)
    print("  TRAINING COMPLETE — SOAP (LOWERCASE)")
...
if __name__ == "__main__":
    ...
    OUTPUT_ROOT.mkdir(parents=True, exist_ok=True)
    sys.stdout = _Tee(OUTPUT_ROOT / f"v3_mnist_letter_lc_soap_{IMG_SIZE}_cli_{datetime.now().strftime('%Y%m%d_%H%M%S')}.txt")
