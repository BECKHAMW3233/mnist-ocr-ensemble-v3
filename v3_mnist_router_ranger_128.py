...
Training data:
  digit (label=0): the SAME merged digit ensemble sources used by
    v3_mnist_digit_*.py at this resolution tier — USPS alone (via
...
Output: ./v3_mnist_router_ranger_128/  (created next to this script)
"""
...
OUTPUT_ROOT = Path(__file__).resolve().parent / f"v3_mnist_router_ranger_{IMG_SIZE}"
...
def build_config(img_size: int, batch_size: int) -> dict:
    prefix = f"v3_mnist_router_ranger_{img_size}"
    return {
        "img_size":    img_size,
...
class OCRRouterNet(nn.Module):
    ...
    Filter progression: 16→32→64→64.
    """
    def __init__(self, num_classes: int = NUM_CLASSES):
        super().__init__()
        self.stem   = ConvBlock(1, 16)
        self.stage1 = nn.Sequential(ConvBlock(16, 32), nn.MaxPool2d(2))
        self.stage2 = nn.Sequential(ConvBlock(32, 64), nn.MaxPool2d(2))
        self.stage3 = nn.Sequential(ConvBlock(64, 64), nn.MaxPool2d(2))
...
def evaluate(model, loader, criterion, device, per_class: bool = False) -> tuple:
    ...
    if per_class and class_total.sum() > 0:
        ...
        print(f"    Balanced accuracy (mean of the three): {balanced_acc*100:.2f}%")
...
def run_training(img_size: int, batch_override: int = None, gpu_id: int = None):
        ...
        if device.type == "cpu" and HAS_PSUTIL:
            ...
            if _free_ram_gb < RAM_RESERVE_GB:
                print(f"  [RAM] Free RAM ({_free_ram_gb:.2f} GB) below the "
                      f"{RAM_RESERVE_GB} GB reserve — stopping training cleanly after epoch {epoch}")
                break
    ...
    print(f"\n{'='*40}")
    print(f"  [{img_size}x{img_size}] Router test accuracy : {test_acc:.4f}  ({test_acc*100:.2f}%)")
    print(f"  [{img_size}x{img_size}] Router test loss     : {test_loss:.4f}")
    print(f"{'='*40}")
...
def classify_image(model_path: str, image_path: str, img_size: int, device_str: str = "cpu",
    ...
    print(f"[Infer] {image_path} → {label}  "
          f"(top={LABEL_MAP[top_idx]} conf={top_conf:.4f}, floor={unsure_floor}, "
          f"digit={probs[0]:.4f} UC={probs[1]:.4f} LC={probs[2]:.4f})")
    return label, probs
...
if __name__ == "__main__":
    ...
    if _args.infer is not None:
        _model_path = str(OUTPUT_ROOT / f"v3_mnist_router_ranger_{IMG_SIZE}_final.pt")
